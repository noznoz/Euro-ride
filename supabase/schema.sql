...
declare t text;
begin
  foreach t in array array['expenses','reservations','completions','photos','announcements','journal'] loop
...
create policy fuel_write on public.fuel for all to authenticated
  using (public.is_approved() and created_by = auth.uid()) with check (public.is_approved() and created_by = auth.uid());

create table if not exists public.checkins (
  rider_id uuid primary key references public.profiles(id) on delete cascade,
  rider_name text, ts bigint, updated_at timestamptz not null default now()
);
